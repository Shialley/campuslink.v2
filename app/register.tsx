...
import { storage } from '../utils/webCompatibility'; // 替换 AsyncStorage 导入
...
const text = {
  EN: {
    ...
    byCreatingAccount: 'by creating a free account',
    emailAddress: 'Email address',
    userName: 'User name',
    password: 'Password',
    confirmPassword: 'Confirm password',
    emailRecommendation: 'Education email is recommended',
    usernamePlaceholder: 'Set the user name you like',
  ...
  CN: {
    getStarted: '创建账号',
    byCreatingAccount: '免费创建新账号',
    emailAddress: '电子邮箱',
    userName: '用户名',
    password: '密码',
    confirmPassword: '确认密码',
    emailRecommendation: '请设置您的邮箱（推荐使用教育邮箱注册）',
    usernamePlaceholder: '设置您喜欢的用户名',
    passwordRequirement: '请设置您的密码，长度为8-20字符',
    confirmPasswordPlaceholder: '请再次输入您设置的密码',
    agreeToTerms: '我已阅读并同意本平台的',
    termsOfService: '服务条款',
    and: '和',
    privacyPolicy: '隐私政策',
    register: '注册',
...
export default function RegisterScreen() {
  ...
  // 使用useFocusEffect在页面每次获得焦点时检查语言设置
  useFocusEffect(
    useCallback(() => {
  ...
  const validateInputs = () => {
    ...
    if (!formData.username.trim()) {
      Alert.alert('Error', t.pleaseEnterUsername);
      return false;
    }
    ...
    if (!formData.confirmPassword.trim()) {
      Alert.alert('Error', t.pleaseConfirmPassword);
      return false;
    }
    ...
    if (!formData.agreeToTerms) {
      Alert.alert('Error', t.pleaseAgreeToTerms);
      return false;
    }
    return true;
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      const registerData = {
        ...
        langs: language === 'CN' ? 'zh-cn' : 'en'
      };

      console.log('Attempting registration...'); // 添加调试日志
      const result = await registerUser(registerData);
      console.log('Registration result:', result); // 添加调试日志

      if (result.success) {
        const successMessage = result.message || (language === 'EN' ? 'Verification email sent. Please check your email to verify your account.' : '验证邮件已发送，请检查您的邮箱以验证账户。');
        
        // 网页版和移动端不同的处理方式
        if (Platform.OS === 'web') {
          // 网页版使用window.alert和window.confirm
          const confirmed = window.confirm(`${t.registerSuccess}\n\n${successMessage}\n\nClick OK to go to login page.`);
          if (confirmed) {
            // 网页端强制刷新到登录页
        ...
        const errorMessage = result.message || (language === 'EN' ? 'Please try again.' : '请重试。');
        
        if (Platform.OS === 'web') {
          window.alert(`${t.registerFailed}\n\n${errorMessage}`);
        } else {
      ...
      const errorMessage = language === 'EN' ? 'Network or server error. Please try again.' : '网络或服务器错误，请重试。';
      
      if (Platform.OS === 'web') {
        window.alert(`${language === 'EN' ? 'Error' : '错误'}\n\n${errorMessage}`);
      } else {
        Alert.alert(language === 'EN' ? 'Error' : '错误', errorMessage);
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.stepContainer}>
        <Text style={styles.stepTitle}>{t.getStarted}</Text>
        <Text style={styles.stepSubtitle}>{t.byCreatingAccount}</Text>
        
        <View style={styles.inputContainer}>
          <Text style={styles.label}>{t.emailAddress}</Text>
          <TextInput
            style={styles.input}
            placeholder={t.emailRecommendation}
        ...
        <View style={styles.inputContainer}>
          <View style={styles.passwordContainer}>
            ...
            <TouchableOpacity 
              ...
              activeOpacity={0.7}
            >
              {confirmPasswordVisible ? <EyeOnIcon /> : <EyeOffIcon />}
            </TouchableOpacity>
          </View>
          {formData.password && formData.confirmPassword && formData.password !== formData.confirmPassword && (
            <Text style={styles.warningText}>{t.passwordsDoNotMatch}</Text>
          )}
        </View>
        ...
        <TouchableOpacity onPress={handleLogin} style={styles.loginContainer}>
          <Text style={styles.footerText}>
            {t.alreadyMember} <Text style={styles.linkText}>{t.logIn}</Text>
          </Text>
        </TouchableOpacity>
